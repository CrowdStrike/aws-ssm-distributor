...
$installerQueryResp = Invoke-RestMethod -Method Get -Uri "${env:SSM_HOST}/sensors/combined/installers/v1?offset=1&limit=1&sort=version&filter=platform:'windows'" -Headers @{'Authorization' = "Bearer ${env:SSM_AUTH_TOKEN}" } -ErrorAction Stop
...
$downloadSensorResp = Invoke-WebRequest -Uri $downloadUrl -OutFile $installerPath -Headers @{'Authorization' = "Bearer ${env:SSM_AUTH_TOKEN}" } -ErrorAction Stop
